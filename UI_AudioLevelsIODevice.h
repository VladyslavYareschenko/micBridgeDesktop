...
public:
    struct AudioFormat
    {
        ...
        std::uint8_t channelsCount;
        std::uint8_t sampleLength;
        bool isSigned;
        bool isLittleEndian;
    };

public:
    AudioInfo(const AudioFormat &format, QObject *parent);
    ~AudioInfo();
    ...
    qint64 bytesAvailable() const { return buffer_.readAvailable(); }
...
private:
    ...
    {
    public:
        static constexpr inline std::size_t ChunkSize = 4096;

        inline RingChunk() noexcept :
            headOffset_(0), tailOffset_(0)
        {
        }

        inline RingChunk(const RingChunk &other) noexcept :
            chunk(other.chunk), headOffset_(other.headOffset_), tailOffset_(other.tailOffset_)
        {
        }

        explicit inline RingChunk(int alloc) :
            chunk(alloc, Qt::Uninitialized), headOffset_(0), tailOffset_(0)
        {
        }

        explicit inline RingChunk(const QByteArray &qba) noexcept :
            chunk(qba), headOffset_(0), tailOffset_(qba.size())
        {
        }

        inline RingChunk &operator=(const RingChunk &other) noexcept
        {
            chunk = other.chunk;
            headOffset_ = other.headOffset_;
            tailOffset_ = other.tailOffset_;
            return *this;
        }
        inline RingChunk(RingChunk &&other) noexcept :
            chunk(other.chunk), headOffset_(other.headOffset_), tailOffset_(other.tailOffset_)
        {
            other.headOffset_ = other.tailOffset_ = 0;
        }
        inline RingChunk &operator=(RingChunk &&other) noexcept
        {
            swap(other);
            return *this;
        }

        inline void swap(RingChunk &other) noexcept
        {
            chunk.swap(other.chunk);
            qSwap(headOffset_, other.headOffset_);
            qSwap(tailOffset_, other.tailOffset_);
        }
        ...
        {
            return tailOffset_ - headOffset_;
        }
        ...
        {
            return chunk.size() - tailOffset_;
        }
        inline const char *data() const
        {
            return chunk.constData() + headOffset_;
        }
        inline char *data()
        {
            if (isShared())
                detach();
            return chunk.data() + headOffset_;
        }

        inline void advance(int offset)
        {
            Q_ASSERT(headOffset_ + offset >= 0);
        ...
        inline void assign(const QByteArray &qba)
        {
            chunk = qba;
            headOffset_ = 0;
            tailOffset_ = qba.size();
        }
        inline void reset()
        {
            headOffset_ = tailOffset_ = 0;
        }
        inline void clear()
        {
            assign(QByteArray());
    ...
    private:
        QByteArray chunk;
        int headOffset_, tailOffset_;
    };

    class RingBuffer
    {
    public:
        explicit inline RingBuffer(std::size_t growth = RingChunk::ChunkSize) :
    ...
    private:
        QVector<RingChunk> buffers;
        qint64 bufferSize;
        int basicBlockSize;
    };

    jnk0le::Ringbuffer<char, 1024 * 32, true> buffer_;
};
